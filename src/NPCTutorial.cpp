...
void NPCTutorial::Update(float dt) {
	if(!GameData::paused) {
		...
		if (underTutorialControl) {
			if (GetAction() == WALK) {
				if (initPos.x > destination.x && associated.box.GetPos().x - destination.x < 0) {
					if (initPos.y > destination.y && associated.box.GetPos().y - destination.y < 0) {
					...
					if (initPos.y == destination.y) {
						associated.box.SetCenter(destination);
						SetAction(IDLE);
					}
				} else if (initPos.x < destination.x && associated.box.GetPos().x - destination.x > 0) {
					if (initPos.y > destination.y && associated.box.GetPos().y - destination.y < 0) {
						associated.box.SetCenter(destination);
						SetAction(IDLE);
					}
				...
				} else if (initPos.x == destination.x) {
					if (initPos.y > destination.y && associated.box.GetPos().y - destination.y < 0) {
						associated.box.SetCenter(destination);
						SetAction(IDLE);
					}
